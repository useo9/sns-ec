...
<form method="GET" class="mb-4 flex gap-3">
    ...
    <a href="{{ route('admin.shops.index') }}" class="px-4 py-2 bg-gray-800 hover:bg-gray-700 text-gray-300 text-sm rounded-lg transition-colors">リセット</a>
</form>

<div class="bg-[#111111] border border-gray-800 rounded-xl overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-800">
        <span class="text-sm text-gray-500">全 {{ $shops->total() }} 件</span>
    </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-black text-gray-500 text-xs uppercase tracking-wide">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left">ショップ名</th>
            ...
            <tbody class="divide-y divide-gray-800">
                @forelse ($shops as $shop)
                    <tr class="hover:bg-gray-900 transition-colors">
                        <td class="px-6 py-4 font-mono text-gray-500 text-xs">{{ $shop->id }}</td>
                        <td class="px-6 py-4 font-medium text-white">
                            <a href="{{ route('admin.shops.show', $shop) }}" class="hover:text-[#0095f6] transition-colors">{{ $shop->name }}</a>
                        </td>
                        <td class="px-6 py-4 text-gray-400">{{ $shop->email }}</td>
                        <td class="px-6 py-4 text-center text-gray-400">{{ $shop->products_count }}</td>
                        ...
                        <td class="px-6 py-4 text-gray-500 text-xs">{{ $shop->created_at->format('Y/m/d') }}</td>
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('admin.shops.show', $shop) }}" class="text-xs text-[#0095f6] hover:underline">詳細</a>
                                <form method="POST" action="{{ route('admin.shops.suspend', $shop) }}">
                                    @csrf
                                    <button type="submit" class="text-xs {{ $shop->isSuspended() ? 'text-green-400 hover:text-green-300' : 'text-orange-400 hover:text-orange-300' }} transition-colors">
                                        {{ $shop->isSuspended() ? '解除' : '停止' }}
                                    </button>
                                </form>
                                <form method="POST" action="{{ route('admin.shops.destroy', $shop) }}"
                                      onsubmit="return confirm('「{{ $shop->name }}」のショップ（アカウント）を削除しますか？')">
                                    @csrf @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center text-gray-600">ショップが見つかりません</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    @if ($shops->hasPages())
        <div class="px-6 py-4 border-t border-gray-800">{{ $shops->links() }}</div>
    @endif
</div>
@endsection
