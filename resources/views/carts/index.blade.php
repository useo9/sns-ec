            ...
            @else
                <div class="bg-white shadow-sm rounded-lg overflow-hidden">
                    <ul class="divide-y divide-gray-100">
                        @foreach ($carts as $cart)
                            <li class="flex items-center gap-4 p-4">
                                ...
                                @else
                                    <div class="w-20 h-20 bg-gray-100 rounded flex items-center justify-center text-gray-400 text-xs shrink-0">
                                        No Image
                                    </div>
                                @endif
                                <div class="flex-1 min-w-0">
                                    <a href="{{ route('products.show', $cart->product) }}"
                                       class="text-sm font-medium text-gray-800 hover:underline truncate block">
                                        {{ $cart->product->title }}
                                    </a>
                    ...
                    <div class="p-4 bg-gray-50 border-t flex items-center justify-between">
                        <div>
                            <span class="text-sm text-gray-600">合計（{{ $carts->count() }}点）</span>
                            <span class="ml-3 text-xl font-bold text-gray-900">¥{{ number_format($total) }}</span>
                        </div>
                        <a href="{{ route('orders.confirm') }}"
                           class="bg-rose-500 hover:bg-rose-600 text-white font-bold py-2 px-6 rounded-lg transition-colors text-sm">
                            購入手続きへ
                        </a>
                    </div>
                </div>
