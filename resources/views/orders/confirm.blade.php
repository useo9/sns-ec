<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">購入確認</h2>
    </x-slot>
        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white shadow-sm rounded-lg overflow-hidden">
                <div class="px-6 py-4 border-b">
                    <h3 class="font-semibold text-gray-800">注文内容</h3>
                </div>
                <ul class="divide-y divide-gray-100">
                    @foreach ($carts as $cart)
                        <li class="flex items-center gap-4 p-4">
                            ...
                            @else
                                <div class="w-16 h-16 bg-gray-100 rounded flex items-center justify-center text-gray-400 text-xs shrink-0">
                                    No Image
                                </div>
                            @endif
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-medium text-gray-800 truncate">{{ $cart->product->title }}</p>
                                @if ($cart->product->brand)
                                    <p class="text-xs text-gray-400 mt-0.5">{{ $cart->product->brand }}</p>
                                @endif
                            </div>
                            <p class="font-bold text-gray-900 shrink-0">¥{{ number_format($cart->product->price) }}</p>
                        </li>
                    @endforeach
                </ul>
                <div class="px-6 py-4 bg-gray-50 border-t flex justify-between items-center">
                    <span class="font-semibold text-gray-700">合計（{{ $carts->count() }}点）</span>
            ...
            <div class="bg-white shadow-sm rounded-lg overflow-hidden">
                <div class="px-6 py-4 border-b">
                    <h3 class="font-semibold text-gray-800">配送先</h3>
                </div>
            ...
            <div>
                <a href="{{ route('carts.index') }}" class="text-sm text-gray-500 hover:underline">← カートへ戻る</a>
            </div>
        </div>
    </div>
</x-app-layout>
